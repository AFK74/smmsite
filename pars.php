...
<body>
	...
	<div class="content">
		...
		<div class="textareas">
			<textarea id="int" rows="15" cols="60" maxlength="2000"></textarea>
			<textarea id="out" rows="15" cols="60" maxlength="2000" placeholder="Результат"></textarea>
		</div>
	</div>
<script>
	function loadinfo(){
		var count = $('#int').val();
		sendRequest('groups.getById',{group_ids: count, fields: 'members_count'}, function(data){
			console.log(data);
			drawList(data.response);
			alert('Готово');
		});
	}
	...
	function drawList(list){
		...
		for (var i = 0; i < list.length; i++){
			...
			html += l.name +' - Подписчиков: ' + l.members_count+'\n';
		}
